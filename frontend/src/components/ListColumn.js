...
export default function ListColumn({ list, tasks, refreshData, boardMembers }) {
  ...
  return (
    <div className="bg-gray-100 min-w-[280px] max-w-[280px] rounded-lg p-3 flex flex-col max-h-full shadow-sm">
      <h2 className="font-bold bg-blue-600 text-white px-2 py-2 text-center mb-3 text-sm uppercase tracking-wide rounded">
        {list.title}
      </h2>

      <div className="flex-grow overflow-y-visible min-h-[50px]">
        
        <SortableContext items={tasks.map(t => t._id)} strategy={verticalListSortingStrategy}>
          {tasks.map((task) => (
            <TaskCard key={task._id} task={task} boardMembers={boardMembers} />
          ))}
        </SortableContext>
      </div>
